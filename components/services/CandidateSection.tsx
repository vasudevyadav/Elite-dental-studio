    ...
    <section className="grid items-center gap-6 rounded-[22px] bg-[#ecfaf7] p-6 lg:grid-cols-[minmax(0,1fr)_minmax(520px,600px)] lg:p-16">
      <div>
        ...
        <ul className="grid gap-[14px] pl-[18px] lg:gap-[10px]">
          {candidates.map((item) => <li className="list-disc text-sm text-gray-600 leading-7 lg:text-[17px] lg:leading-8" key={item}>{item}</li>)}
        </ul>
        <strong className="mt-7 block text-lg font-bold italic leading-[1.55]">Suitability is confirmed after a clinical examination and dental X-ray at our clinic.</strong>
      </div>
