...
export default function ContactFormSection() {
  ...
  const fieldClass = "w-full rounded-xl border border-[#c7dfdc] bg-[#f7fbfa] px-4 py-3.5 text-sm text-[#324e51] outline-none transition placeholder:text-[#8a999a] hover:border-[#8fc7c1] focus:border-[#25bfae] focus:bg-white focus:ring-4 focus:ring-[#25bfae]/15";
    ...
    <section id="contact-form" className="bg-white px-4 py-12 sm:px-8 sm:py-16 lg:py-16">
      <div className="mx-auto grid max-w-7xl items-center overflow-hidden rounded-[22px] bg-[linear-gradient(145deg,#174e53,#2a7175)] shadow-[0_24px_65px_rgba(18,75,79,.18)] sm:rounded-[30px] lg:grid-cols-[.88fr_1.12fr]">
        <div className="relative overflow-hidden p-6 text-white sm:p-10 lg:p-12 xl:p-16">
          ...
          <h2 className="mt-4 text-[28px] font-semibold leading-tight tracking-[-.04em] sm:text-4xl xl:text-5xl">Let’s make your next dental visit easy.</h2>
          ...
          <div className="relative mt-10 grid gap-4 text-sm text-white/90 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <div className="rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur-sm"><span className="block text-xs uppercase tracking-[.14em] text-[#51dfd1]">Working hours</span><strong className="mt-3 block text-sm">Mon–Sat: 9:30 AM–9:00 PM</strong><strong className="mt-1 block text-sm">Sunday: 10:00 AM–7:00 PM</strong></div>
            <div className="rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur-sm"><span className="block text-xs uppercase tracking-[.14em] text-[#51dfd1]">Emergency contact</span><a href="tel:+919048611911" className="mt-3 block text-lg font-bold transition hover:text-[#51dfd1]">+91 9048 611 911</a><small className="mt-1 block text-white/60">Tap to call our team</small></div>
          </div>
        </div>
        <div className="m-2 rounded-[18px] bg-white p-5 sm:m-4 sm:rounded-[24px] sm:p-8 lg:m-5 lg:p-10 xl:p-12">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.15em] text-[#25a99d]">Appointment request</p><h3 className="mt-2 text-2xl font-semibold text-[#174e53] sm:text-3xl">Send an enquiry</h3></div><span className="hidden h-12 w-12 place-items-center rounded-full bg-[#e4f7f4] text-xl text-[#188e84] sm:grid">✦</span></div>
          <p className="mt-2 text-sm leading-6 text-[#6d7c7e]">We usually respond during clinic working hours.</p>
          <form className="mt-7 grid gap-4 sm:grid-cols-2 lg:gap-5" onSubmit={(event) => event.preventDefault()}>
            ...
            <button type="submit" className="smooth-hover button-hover mt-2 w-full rounded-xl bg-[#25bfae] px-7 py-4 text-sm font-bold uppercase tracking-[.08em] text-white hover:bg-[#176b70] sm:col-span-2 sm:w-fit">Submit enquiry →</button>
          </form>
        </div>
      </div>
    </section>
