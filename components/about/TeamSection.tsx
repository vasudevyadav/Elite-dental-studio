  ...
  return (
    <section className="px-5 py-16 sm:px-8 lg:py-28">
      <div className="mx-auto max-w-[1120px] rounded-[24px] bg-[#2c7477] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{management.map((person) => <article key={person.name} className="overflow-hidden rounded-[13px] bg-white text-center"><Image src={person.image} alt={person.name} width={277} height={239} className="aspect-[1.16] w-full object-cover" /><div className="bg-[#2a7275] px-2 py-3 text-white"><h3 className="text-[10px] font-extrabold sm:text-xs">{person.name}</h3><p className="mt-1 text-[7px] sm:text-[8px]">{person.role}</p></div></article>)}</div>
      </div>
    </section>
  );
}
