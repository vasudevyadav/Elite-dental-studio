  ...
  return (
    <section className="px-5 py-16 sm:px-8 lg:py-20">
      <div className="mx-auto max-w-7xl rounded-[24px] bg-dent-panel px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
        <h2 className="text-center text-2xl font-bold text-white lg:text-4xl">Meet Our Directors</h2>
        <p className="mx-auto mt-3 max-w-2xl text-center text-xs lg:text-base leading-5 text-white/90">Our leadership combines clinical experience, thoughtful management and a shared commitment to exceptional patient care.</p>
        ...
        <p className="mx-auto mt-3 max-w-2xl text-center text-xs lg:text-base leading-5 text-white/90">Meet the people who keep every clinic experience welcoming, organised and focused on your care.</p>
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{management.map((person) => <article key={person.name} className="overflow-hidden rounded-[13px] bg-white text-center"><Image src={person.image} alt={person.name} width={277} height={239} className="aspect-[1.16] w-full object-cover" /><div className="bg-dent-panel px-2 py-3 text-white"><h3 className="text-[10px] font-extrabold lg:text-lg">{person.name}</h3><p className="mt-1 text-xs sm:text-sm">{person.role}</p></div></article>)}</div>
      </div>
    </section>
  );
}
